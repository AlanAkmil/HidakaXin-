...
async function getData(animeId) {
  try {
    const detail = await samehadaku.detail(animeId);
    return { detail };
  } catch {
    return { detail: null };
  }
...
export default async function AnimeSamehadakuDetailPage({ params }) {
  ...
  // Samehadaku's detail() sometimes returns an empty `title` — the real
  // title lives in `english`/`japanese` instead.
  const displayTitle = d.title || d.english || d.japanese || 'Tanpa Judul';
  // episodeList[].title here is a bare episode number (e.g. 1173), not a
  // string like Otakudesu's — sort numerically and render "Episode N".
  const sortedEpisodes = [...(d.episodeList || [])].sort((a, b) => (Number(b.title) || 0) - (Number(a.title) || 0));
    ...
    <div className="mx-auto max-w-3xl px-4 py-6">
      <div className="grid gap-6 sm:grid-cols-[200px_1fr]">
        <RevealOnView>
          <div className="overflow-hidden rounded-xl border border-line shadow-card">
            {d.poster ? (
              ...
              <img src={d.poster} alt={displayTitle} className="w-full object-cover" />
            ) : (
        ...
        <div>
          ...
          <h1 className="font-display text-2xl font-extrabold text-ink sm:text-3xl">{displayTitle}</h1>
        ...
        <div className="mt-8">
          ...
          <div className="max-h-[480px] space-y-1.5 overflow-y-auto pr-1">
            {sortedEpisodes.map((ep) => (
              ...
              >
                <span className="font-semibold text-ink">{ep.title ? `Episode ${ep.title}` : 'Episode'}</span>
        ...
        <div className="mt-8">
          ...
          <div className="grid grid-cols-3 gap-3 sm:grid-cols-5">
            {d.recommendedAnimeList.map((r) => (
              <Link key={r.animeId} href={`/anime-same/${r.animeId}`} className="group">
