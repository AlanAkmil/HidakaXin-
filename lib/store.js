...
export async function addCoins(amount) {
  ...
  const { error } = await supabase.from('coins').upsert({ user_id: uid, balance: next, updated_at: new Date().toISOString() });
  if (error) return null;
  notify(KEYS.coins);
  return next;
}
...
export async function buyOrSelectBanner(bannerId) {
  ...
  if (!owned.includes(bannerId)) {
    ...
    const newBalance = await addCoins(-item.cost);
    if (newBalance === null) return { ok: false, error: 'Gagal menyimpan ke server, coba lagi' };
    if (!uid) {
      write(KEYS.ownedBanners, [...owned, bannerId]);
    } else {
      const { error } = await supabase.from('owned_banners').insert({ user_id: uid, banner_id: bannerId });
      if (error) {
        await addCoins(item.cost); // refund since the purchase didn't actually go through
        return { ok: false, error: 'Gagal menyimpan banner, coin dikembalikan' };
      }
    }
  }
  ...
  if (!uid) {
    ...
    const { error } = await supabase.from('profile_settings').upsert({ user_id: uid, selected_banner: bannerId });
    if (error) return { ok: false, error: 'Gagal menyimpan pilihan banner' };
  }
...
export async function claimCheckin() {
  ...
  if (!uid) {
    ...
    const { error } = await supabase.from('checkins').upsert({ user_id: uid, streak, last_date: today });
    if (error) return { ok: false, error: 'Gagal simpan check-in ke server, coba lagi (' + error.message + ')' };
  }
  const newBalance = await addCoins(reward);
  if (newBalance === null) return { ok: false, error: 'Check-in kesimpen tapi koin gagal ditambah, coba refresh' };
  notify(KEYS.checkin);
...
export async function claimMission(missionId) {
  ...
  if (!uid) {
    ...
    if (error) return { ok: false, error: 'Gagal simpan klaim ke server, coba lagi (' + error.message + ')' };
  }
  const newBalance = await addCoins(m.reward);
  if (newBalance === null) return { ok: false, error: 'Misi kesimpen tapi koin gagal ditambah, coba refresh' };
  notify(KEYS.missionProgress);
