...
// Otakudesu's home() returns { ongoing: {animeList}, completed: {animeList} }.
// Samehadaku's home() has a totally different shape: { recent: {animeList},
// top10: {animeList}, movie, batch } — no ongoing/completed at all. This
// normalizes both into the { ongoing, completed } pair the pages expect,
// using `recent` as the ongoing-equivalent and `top10` (ranked/scored) as
// the completed/popular-equivalent for Samehadaku.
// Otakudesu's schedule() returns a flat array: [{ day: 'Senin', anime_list:
// [...] }, ...]. Samehadaku wraps it one level deeper and uses English day
// names + camelCase field: { days: [{ day: 'Saturday', animeList: [...] }] }.
// This normalizes both into the flat [{ day, animeList }] shape the jadwal
// page iterates over. English day names still resolve fine downstream since
// findScheduleKeyForDay already has English aliases built in.
export function extractSankaScheduleEntries(data, provider = 'otakudesu') {
  if (provider === 'samehadaku') {
    return (data?.days || []).map((entry) => ({ day: entry.day, animeList: entry.animeList || [] }));
  }
  return (data || []).map((entry) => ({ day: entry.day, animeList: entry.anime_list || [] }));
}

export function extractSankaHomeLists(data, provider = 'otakudesu') {
  if (provider === 'samehadaku') {
    return {
      ongoing: data?.recent?.animeList || [],
      completed: data?.top10?.animeList || []
    };
  }
  return {
...
export function normalizeSanka(item, provider = 'otakudesu') {
  const id = item.animeId || item.slug;
  return {
    source: provider === 'samehadaku' ? 'samehadaku' : 'sanka',
    title: item.title,
    url: id,
    image: item.poster,
    type: null,
